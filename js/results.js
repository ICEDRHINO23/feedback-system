...
async function loadExamMap() {
    ...
    snapshot.forEach(s => { examMap[s.id] = { id: s.id, ...s.data() }; });
}
...
function getExam(result) { return examMap[result.examId] || {}; }
function getStudent(result) { return result.studentName || result.participantName || "-"; }
function getExamName(result) { const e=getExam(result); return e.examName || result.examName || "-"; }
function getSubject(result) { const e=getExam(result); return e.subject || result.subject || "-"; }
function getClass(result) { const e=getExam(result); return e.examClass || result.examClass || result.studentClass || "-"; }
...
function renderResults(results) {
    ...
    tbody.innerHTML = results.map((result,index) => {
        ...
        return `<tr>
            ...
            <td>${escapeHtml(getExamName(result))}</td>
            <td>${escapeHtml(getSubject(result))}</td>
            <td>${escapeHtml(getClass(result))}</td>
...
function updateDashboard(results) {
    const students = new Set(results.map(getStudent));
    const percentages = results.map(r => Number(r.percentage || 0));
    const attempts = results.length;
    const total = percentages.reduce((a,b)=>a+b,0);
    const passed = percentages.filter(p=>p>=35).length;
...
function filterResults() {
    ...
    const filtered=allResults.filter(r=>{
        const text=`${getStudent(r)} ${getExamName(r)} ${getSubject(r)}`.toLowerCase();
        return text.includes(search) && (!cls||getClass(r)===cls) && (!exam||getExamName(r)===exam) && (!subject||getSubject(r)===subject);
    });
...
window.deleteResult = async function(id) {
    ...
    try { await deleteDoc(doc(db,"results",id)); await loadResults(); }
    catch(error){ console.error("DELETE ERROR:",error); alert("Unable To Delete Result"); }
};

function exportExcel(){
    if(!displayedResults.length){alert("No results available to export.");return;}
    const rows=displayedResults.map((r,i)=>`<tr><td>${i+1}</td><td>${escapeHtml(getStudent(r))}</td><td>${escapeHtml(getExamName(r))}</td><td>${escapeHtml(getSubject(r))}</td><td>${escapeHtml(getClass(r))}</td><td>${escapeHtml(getSection(r))}</td><td>${Number(r.score||0)}</td><td>${Number(r.totalMarks||0)}</td><td>${Number(r.percentage||0).toFixed(2)}%</td><td>${r.submittedAt?new Date(r.submittedAt).toLocaleString("en-IN"):"-"}</td></tr>`).join("");
    const html=`<html><head><meta charset="UTF-8"></head><body><h2>Academic Heights Public School</h2><h3>AHPS Assessment Results</h3><table border="1"><tr><th>Sr No</th><th>Student</th><th>Assessment</th><th>Subject</th><th>Class</th><th>Section</th><th>Score</th><th>Total Marks</th><th>Percentage</th><th>Date</th></tr>${rows}</table></body></html>`;
    const url=URL.createObjectURL(new Blob([html],{type:"application/vnd.ms-excel;charset=utf-8"}));
    const a=document.createElement("a");a.href=url;a.download=`AHPS_Assessment_Results_${new Date().toISOString().slice(0,10)}.xls`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);
}
...
async function exportPDF(){
    if(!displayedResults.length){alert("No results available to export.");return;}
    if(!window.jspdf?.jsPDF){alert("PDF library is not loaded. Please refresh the page.");return;}
    const {jsPDF}=window.jspdf; const pdf=new jsPDF({orientation:"landscape",unit:"mm",format:"a4"});
    const w=pdf.internal.pageSize.getWidth(), h=pdf.internal.pageSize.getHeight();
    pdf.setFillColor(0,31,91);pdf.rect(0,0,w,32,"F");
    const loadImage=src=>new Promise(resolve=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=()=>resolve(null);img.src=src;});
    const ahps=await loadImage("../assets/ahps-logo.png"), ras=await loadImage("../assets/ras-logo.jpeg");
    if(ahps)pdf.addImage(ahps,"PNG",9,4,24,24);if(ras)pdf.addImage(ras,"JPEG",w-33,4,24,24);
    pdf.setTextColor(255,255,255);pdf.setFont("helvetica","bold");pdf.setFontSize(17);pdf.text("ACADEMIC HEIGHTS PUBLIC SCHOOL",w/2,11,{align:"center"});
    pdf.setFont("helvetica","normal");pdf.setFontSize(9);pdf.text("Chikhali, Pune, Maharashtra",w/2,17,{align:"center"});pdf.setFont("helvetica","bold");pdf.setFontSize(10);pdf.text("ASSESSMENT PERFORMANCE REPORT",w/2,25,{align:"center"});
